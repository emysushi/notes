
# le son, comment se propage-t-il?

un signal sonore est produit par la vibration d'un objet. La présence d'une caisse de résonance amplifie le signal. Un signal sonore a besoin d'un milieu materiel pour se propager
Une onde sonore est une one mécanique progressive , elle transporte de l'énergie

# vitesse de propagation 

on a la formule : v = d/t (vitesse en m/s, distance en m, temps en sec)
la vitesse de propagation d'un signal dépend du milieu de propagation et de sa température 
air < eau < cuivre 

___

| Téra  | Giga | Méga | Kilo | Hecto | Déca | (unité) | Déci  | Centi | Milli | Micro | Nano  |
| ----- | ---- | ---- | ---- | ----- | ---- | ------- | ----- | ----- | ----- | ----- | ----- |
| 10^12 | 10^9 | 10^6 | 10^3 | 10^2  | 10^1 |         | 10^-1 | 10^-2 | 10^-3 | 10^-6 | 10^-9 |

___

# TP : mesure de la vitesse du son dans l'air

valeur théorique de la vitesse du son dans l'air : Vson = 331,5 + 0,607 * T

- utilisation de :
	- l'oscilloscope (mesure et d'affiche une tension en fonction du temps)
	- du générateur (créent des formes d'ondes électroniques en convertisant l'énergie produite)
	- l'émetteur ( transforme le signal contenant l'info en onde radioélectrique de puissance suffisante pour assurant la liaison à un récepteur)